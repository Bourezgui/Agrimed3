package com.example.ynaccache.agrimed2.activity.views;
...
public class DotsProgressBar extends View {
	...
	private void init(Context context) {
		mRadius = context.getResources().getDimension(
				R.dimen.circle_indicator_radius);
		// dot fill color
		mPaintFill.setStyle(Style.FILL);
		mPaintFill.setColor(getResources().getColor(R.color.colorPrimary));
		// dot background color
		mPaint.setStyle(Style.FILL);
		mPaint.setColor(0x33245b9a);
		start();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		float dX = (widthSize - mDotCount * mRadius * 2 - (mDotCount - 1)
				* margin) / 2.0f;
